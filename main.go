...
type runConfig struct {
	...
	DisneyIP					string		`json:"disneyIP"`
	NetflixIP					string		`json:"netflixIP"`
	BindDnsOverHttps			bool		`json:"bindDnsOverHttps"`
	DoHPort						int			`json:"DoHPort"`
	DNSQueryPath				string		`json:"DNSQueryPath"`
	JSONQueryPath				string		`json:"JSONQueryPath"`
	DNSQueryAuth				bool		`json:"DNSQueryAuth"`
	JSONQueryAuth				bool		`json:"JSONQueryAuth"`
	SNIProxy					bool		`json:"SNIProxy"`
...
func main() {
	...
	flag.IntVar(&c.DoHPort, "DoHPort", 0, "DoHPort")
	flag.BoolVar(&c.BindDnsOverHttps, "bindDnsOverHttps", false, "enable DNS over HTTPS as well as UDP")
	flag.BoolVar(&c.DNSQueryAuth, "DNSQueryAuth", false, "enable DnsQueryAuthP")
	flag.BoolVar(&c.JSONQueryAuth, "JSONQueryAuth", false, "enable JSONQueryAuth")
	flag.BoolVar(&c.SNIProxy, "SNIProxy", true, "enable SNIproxy")
	flag.StringVar(&c.DNSQueryPath, "DNSQueryPath", "/dns-query", "Default /dns-query")
	flag.StringVar(&c.JSONQueryPath, "JSONQueryPath", "/resolve", "Default /resolve")
	flag.StringVar(&c.Authorization, "Authorization", "Authorization", "Authorizationg")
	flag.StringVar(&c.DomainListPath, "HTTPProxy", "", "HTTPProxy")

	Version := flag.BoolP("version", "v", false, "Print program version information")

	flag.Parse()

	if *Version {
		fmt.Printf("SNIProxy v%s Build:%s\n", version, buildTime)
		return
	}
	...
	if c.DisneyIP == "" {
		c.DisneyIP = c.PublicIP
	}
	if c.NetflixIP == "" {
		c.NetflixIP = c.PublicIP
	}
